 ...
 * Two k-itemsets can be joined if their first (k-1) items are IDENTICAL.
 */
function aprioriGen(freqK: string[][], freqKeySet: Set<string>): string[][] {
  const candidates: string[][] = [];
  // Ensure each itemset is sorted
  ...
  for (let i = 0; i < sorted.length; i++) {
    for (let j = i + 1; j < sorted.length; j++) {
      ...
      // Check if first (k-1) elements are identical element-by-element
      ...
      // Last element of a must be strictly less than last of b (avoids duplicates)
      if (a[k - 1] >= b[k - 1]) continue;

      const candidate = [...a, b[k - 1]]; // already sorted

      // Prune: every (k)-subset of candidate must be frequent
      let valid = true;
      // Only need to check (k+1) choose k subsets — the ones that drop one element
...
export function runApriori(
  ...
  const minCount = Math.ceil(minSupport * N); // minimum absolute count

  // ── Step 1: Find frequent 1-itemsets ────────────────────────────────────────
  const allItems = new Set<string>();
  txArrays.forEach((tx) => tx.forEach((item) => allItems.add(item)));

  // supportMap: itemKey → absolute count
  ...
  for (let k = 2; k <= 8; k++) {
    ...
    // Build set of prev frequent keys for pruning
